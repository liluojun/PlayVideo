...
std::string YUV_FRAGMENT_SHADER_STRING =
        ...
        "vec3 yuv;\n"
        "vec3 rgb;\n"
        "yuv.x = texture2D(tex_y, interp_tc).r;\n"
        "yuv.y = texture2D(tex_u, interp_tc).r - 0.5;\n"
        "yuv.z = texture2D(tex_v, interp_tc).r - 0.5;\n"
        "rgb = mat3( 1, 1, 1,\n"
        "0, -0.39465, 2.03211,\n"
...
int* GlDraw::perparDrawYuv(int width, int height, YuvData *data, int yuvTextures[3]) {
...
void GlDraw::drawYuv(int yuvTextures[3],
...
//
void GlDraw::drawYuv(int yuvTextures[3], float *texMatrix,
                     int viewportX, int viewportY, int viewportWidth, int viewportHeight) {
...
//    } else {
      ...
      //  shader->texMatrixLocation = texMatrixLocation;
        ...
        if (YUV_FRAGMENT_SHADER_STRING == fragmentShader) {
            //对这几个纹理采样器变量进行设置
            glUniform1i(shader->glShader.getUniformLocation("tex_y"), 0);
            glUniform1i(shader->glShader.getUniformLocation("tex_u"), 1);
            glUniform1i(shader->glShader.getUniformLocation("tex_v"), 2);
        }
